...
<style>
    ...
    .badge { display:inline-block; padding:5px 12px; border-radius:999px; font-size:11px; font-weight:bold; background:#FEE2E2; color:#B91C1C; }
...
<div class="page">
    ...
    <div class="content">
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td>
                        ...
                        <p class="brand-sub">Digital Academy Cohort — Payment Receipt</p>
                    </td>
        ...
        <div class="congrats">
            <h2>Payment confirmed &mdash; slot secured! &#127881;</h2>
            <p>Thank you for enrolling in the Digital Academy Cohort Programme</p>
        </div>

        <div class="card">
            <div class="card-head">Enrollment Information</div>
            <div class="card-body">
                <table class="meta">
                    <tr><td class="meta-label">Student:</td><td>{{ $studentName }}</td></tr>
                    <tr><td class="meta-label">Email Address:</td><td>{{ $enrollment->customer->email }}</td></tr>
                    <tr><td class="meta-label">Batch:</td><td>{{ $enrollment->intake->name }} ({{ $enrollment->intake->start_date->format('M j') }}&ndash;{{ $enrollment->intake->end_date->format('M j, Y') }})</td></tr>
                    <tr><td class="meta-label">Course Track:</td><td>{{ $enrollment->track_selected }}</td></tr>
                    <tr><td class="meta-label">Programme:</td><td>{{ $enrollment->programme_selected }}</td></tr>
                    <tr><td class="meta-label">Training Package:</td><td><span class="badge">{{ $enrollment->bootcamp_option === 'bootcamp' ? 'Bootcamp Training' : 'Non-Bootcamp Training' }}</span></td></tr>
        ...
        <div class="card">
            <div class="card-head">Fee Breakdown</div>
            <div class="card-body">
                <table class="details">
                    <thead>
                    ...
                    <tbody>
                        <tr>
                            <td>{{ $enrollment->bootcamp_option === 'bootcamp' ? 'Bootcamp Training Fee' : 'Non-Bootcamp Training Fee' }}</td>
                            <td>&#8358;{{ number_format($enrollment->bootcamp_fee, 2) }}</td>
                        </tr>
                        <tr>
                            <td>Tuition Fee{{ $enrollment->tuition_tier === 'discounted' ? ' (50% discount applied)' : '' }}</td>
                            <td>&#8358;{{ number_format($enrollment->tuition_fee, 2) }}</td>
                        </tr>
                    </tbody>
                </table>
                <div class="amount-box">
                    <div class="amount-paid">Total Paid: &#8358;{{ number_format($enrollment->amount_paid, 2) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            Digital Academy Cohort Programme. Reference: {{ $enrollment->reference }}
        </div>
    </div>
</div>
</body>
